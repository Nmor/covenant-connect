...
def _log_sermon_engagement(member: Member, sermon: Sermon) -> bool:
    ...
    interaction = CareInteraction(
        ...
        metadata_json={'sermon_id': sermon.id, 'sermon_title': sermon.title},
    )
...
def sermons():
    try:
        ...
        preacher = (request.args.get('preacher') or '').strip()
        ...
        flash('An unexpected error occurred while loading sermons.', 'danger')
        return render_template('sermons.html', sermons=[])
...
def sermon_detail(sermon_id: int):
    try:
        ...
        related_sermons = (
            Sermon.query.filter(Sermon.id != sermon_id)
        ...
        if member:
            try:
                ...
                current_app.logger.error(
                    'Database error logging sermon engagement for member %s: %s',
                    member.id,
                    engagement_exc,
                )
            except Exception as engagement_exc:  # pragma: no cover - defensive logging
                db.session.rollback()
                current_app.logger.error(
                    'Unexpected error logging sermon engagement for member %s: %s',
                    member.id,
                    engagement_exc,
                )
        ...
        current_app.logger.error('Database error while fetching sermon %s: %s', sermon_id, exc)
        flash('Unable to load the sermon right now. Please try again later.', 'danger')
        return redirect(url_for('sermons.sermons'))
    except Exception as exc:  # pragma: no cover - defensive logging
        current_app.logger.error('Unexpected error in sermon_detail route: %s', exc)
