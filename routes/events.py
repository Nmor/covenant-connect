...
from app import db
from models import AttendanceRecord, Event, FacilityReservation, VolunteerAssignment, VolunteerRole
...
def _safe_filename(event: Event, suffix: str) -> str:
    base = ''.join(
        char if char.isalnum() or char in (' ', '_', '-') else '_' for char in event.title.lower()
    )
...
def _collect_event_resource_data(event: Event):
    reservations = (
        FacilityReservation.query.filter_by(event_id=event.id)
        .order_by(FacilityReservation.start_time)
        .all()
    )

    total_capacity = 0
    resource_allocations: list[dict[str, object]] = []
    resource_totals: dict[int, dict[str, object]] = {}
    ...
    for reservation in reservations:
        ...
        for allocation in reservation.resource_requests:
            ...
            entry = resource_totals.setdefault(
                resource.id,
    ...
    return reservations, resource_allocations, resource_totals, total_capacity


def _filter_events_by_tag(events: Iterable[Event], tag: str | None) -> list[Event]:
    if not tag:
        return list(events)
    tag_lower = tag.lower()
    return [
...
def events():
    ...
    try:
        ...
        return render_template('events.html', events=[], tag_filter=tag_filter, available_tags=[])
...
def event_detail(event_id: int):
    try:
        ...
        upcoming_events = (
            Event.query.filter(Event.start_date >= datetime.utcnow(), Event.id != event.id)
            .order_by(Event.start_date)
            .limit(3)
            .all()
        )
        ...
        current_app.logger.error('Database error while fetching event %s: %s', event_id, exc)
        flash('Unable to load the event at this time. Please try again later.', 'danger')
        return redirect(url_for('events.events'))
    except Exception as exc:  # pragma: no cover - defensive logging
        current_app.logger.error('Unexpected error in event_detail route: %s', exc)
        flash('An unexpected error occurred. Please try again later.', 'danger')
        return redirect(url_for('events.events'))


@events_bp.route('/events/<int:event_id>/attendance.csv')
def export_attendance_csv(event_id: int):
    try:
        ...
        current_app.logger.error(
            'Database error exporting attendance for event %s: %s', event_id, exc
        )
        flash('Unable to export attendance records right now.', 'danger')
        return redirect(url_for('events.event_detail', event_id=event_id))
    except Exception as exc:  # pragma: no cover - defensive logging
        current_app.logger.error(
            'Unexpected error exporting attendance for event %s: %s', event_id, exc
        )
...
def export_attendance_excel(event_id: int):
    try:
        event = db.session.get(Event, event_id)
        if not event:
            flash('Event not found.', 'warning')
        ...
        <html>
            ...
            <body>
                ...
                <p>Total facility capacity: {{ total_capacity }}</p>
            </body>
        </html>
        """
        ...
        current_app.logger.error(
            'Database error exporting Excel for event %s: %s', event_id, exc
        )
        flash('Unable to export Excel follow-up right now.', 'danger')
        return redirect(url_for('events.event_detail', event_id=event_id))
    except Exception as exc:  # pragma: no cover - defensive logging
        current_app.logger.error(
            'Unexpected error exporting Excel for event %s: %s', event_id, exc
        )
...
def plan_event(event_id: int):
    ...
    if request.method == 'POST':
        ...
        if recurrence_end_input:
            try:
                event.recurrence_end_date = datetime.strptime(recurrence_end_input, '%Y-%m-%d')
            except ValueError:
        ...
        event.ministry_tags = [tag.strip() for tag in raw_tags.split(',') if tag.strip()]
        ...
        try:
            ...
            db.session.rollback()
            current_app.logger.error('Database error saving plan for event %s: %s', event_id, exc)
            flash('An error occurred while updating the event plan. Please try again.', 'danger')

    segments_for_form = event.service_segments or [
        {'title': '', 'leader': '', 'duration': '', 'notes': ''}
    ]
    planned_segments = [
        segment for segment in event.service_segments or [] if any(segment.values())
    ]
...
def departmental_calendar():
    try:
        ...
        return render_template('calendar/index.html', grouped_events=grouped_events)
    except SQLAlchemyError as exc:
        ...
        return render_template('calendar/index.html', grouped_events={})
...
def events_ical_feed():
    ...
    for event in events_filtered:
        ...
        if event.ministry_tags:
            category_line = ','.join(_escape_ics_text(tag) for tag in event.ministry_tags)
