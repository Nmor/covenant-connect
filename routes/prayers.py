"""Prayer request submission and listing views."""
...
def _record_prayer_interaction(member: Member, prayer: PrayerRequest, is_public: bool) -> None:
    ...
    interaction = CareInteraction(
        ...
        source='prayer_form',
        extra_data={'prayer_request_id': prayer.id, 'is_public': is_public},
    )

    member.last_interaction_at = interaction_time
    if follow_up_required and (
        not member.next_follow_up_date or follow_up_date < member.next_follow_up_date
    ):
...
def _enqueue_notification(prayer_id: int) -> None:
    ...
    try:
        if queue:
            queue.enqueue(send_prayer_notification, prayer_id)
        else:
            send_prayer_notification(prayer_id)
    except Exception as exc:  # pragma: no cover - best effort background task
        current_app.logger.warning('Unable to queue prayer notification: %s', exc)
...
def prayers() -> str:
    ...
    return render_template('prayers.html', public_prayers=public_prayers)
...
def submit_prayer():
    ...
    prayer = PrayerRequest(name=name, email=email, request=prayer_text, is_public=is_public)
    ...
    try:
        ...
        if member:
            _record_prayer_interaction(member, prayer, is_public)

        db.session.commit()
    except SQLAlchemyError as exc:
        db.session.rollback()
        current_app.logger.error('Database error submitting prayer request: %s', exc)
        flash('An error occurred while submitting your prayer request. Please try again.', 'danger')
        return redirect(url_for('prayers.prayers'))

    triggered = trigger_automation('prayer_request_created', {'prayer_request_id': prayer.id})

    if triggered == 0:
        _enqueue_notification(prayer.id)
